...
def normalize_material(raw: Any) -> str | None:
    ...
    normalized = MATERIAL_MAP.get(text, text)

    if normalized not in {"CAN", "PET", "GLASS"}:
        return None
...
class EnvipcoCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def throttle_status_text(self) -> str:
        if self.stats_throttled and self.rejects_throttled:
            return "rvmStats + rejects throttled"
        if self.stats_throttled:
            return "rvmStats throttled"
        if self.rejects_throttled:
            return "rejects throttled"
        return "ok"
    ...
    def machine_address(self, rvm_id: str) -> str | None:
        meta = self.machine_meta(rvm_id)
        return str(
            meta.get("address") or self.rvm_data(rvm_id).get("SiteInfoAddress") or ""
        ).strip() or None

    def machine_postal_code(self, rvm_id: str) -> str | None:
        meta = self.machine_meta(rvm_id)
        return str(
            meta.get("postal_code") or self.rvm_data(rvm_id).get("SiteInfoPostalCode") or ""
        ).strip() or None

    def machine_city(self, rvm_id: str) -> str | None:
        meta = self.machine_meta(rvm_id)
        return str(
            meta.get("city") or self.rvm_data(rvm_id).get("SiteInfoCity") or ""
        ).strip() or None

    def machine_country(self, rvm_id: str) -> str | None:
        meta = self.machine_meta(rvm_id)
        return str(
            meta.get("country") or self.rvm_data(rvm_id).get("SiteInfoCountry") or ""
        ).strip() or None
    ...
    def _bin_api_limit(self, rvm_id: str, bin_no: int) -> int:
        return self.safe_int(self.rvm_data(rvm_id).get(f"{BIN_LIMIT_PREFIX}{bin_no}"))

    def _bin_count(self, rvm_id: str, bin_no: int) -> int:
        return self.safe_int(self.rvm_data(rvm_id).get(f"{BIN_COUNT_PREFIX}{bin_no}"))

    def _bin_full(self, rvm_id: str, bin_no: int) -> int:
        return self.safe_int(self.rvm_data(rvm_id).get(f"{BIN_FULL_PREFIX}{bin_no}"))

    def _bin_is_disabled_marker(self, rvm_id: str, bin_no: int) -> bool:
        """Hard filter for bins that are not used by the machine.

        Observed value:
        - BinInfoFullBinX == -2  -> not configured / not used
        """
        return self._bin_full(rvm_id, bin_no) == -2

    def _bin_is_active(self, rvm_id: str, bin_no: int) -> bool:
        if self._bin_is_disabled_marker(rvm_id, bin_no):
            return False
    ...
    def active_bins(self, rvm_id: str) -> list[int]:
        ...
        for bin_no in range(1, 13):
            if self._bin_is_active(rvm_id, bin_no):
                active.append(bin_no)

        _LOGGER.warning(
            "Envipco RVM %s actieve bins bepaald als: %s",
            rvm_id,
            active,
        )
        return active
    ...
    def current_bin_limit(self, rvm_id: str, bin_no: int) -> int | None:
        ...
        api_value = self._bin_api_limit(rvm_id, bin_no)
        if api_value > 0:
            return api_value
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        for row in rejects_rows:
            ...
            for key in REJECT_KEYS:
                rejects_by_machine[machine_id][key] += self.safe_int(
                    self._row_get_case_insensitive(row, key)
                )
