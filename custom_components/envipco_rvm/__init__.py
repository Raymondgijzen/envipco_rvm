...
def _current_active_bins_map(coordinator: EnvipcoCoordinator) -> dict[str, tuple[int, ...]]:
    """Return active bins per machine."""
...
async def _async_apply_registry_naming(
    ...
    """Normalize entity and device naming in the registries."""
    ...
    for device in list(device_registry.devices.values()):
        ...
        if (DOMAIN, f"{entry.entry_id}_platform") in identifiers:
...
async def _async_force_remove_inactive_bin_entities(
    ...
    """Remove inactive bin entities from the entity registry."""
    entity_registry = er.async_get(hass)
...
async def _async_process_coordinator_update(
    ...
    """Trigger reload when active bin layout changes."""
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload Envipco RVM config entry."""
...
async def _async_handle_entry_update(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Handle config-entry option updates by doing a normal HA reload."""
    await hass.config_entries.async_reload(entry.entry_id)
