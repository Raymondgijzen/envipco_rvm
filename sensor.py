...
class EnvipcoCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def _bin_full(self, rvm_id: str, bin_no: int) -> Any:
        return self.rvm_data(rvm_id).get(f"{BIN_FULL_PREFIX}{bin_no}")

    def _bin_is_active(self, rvm_id: str, bin_no: int) -> bool:
        """Return True only for bins that are really configured/in use.

        Rules:
        - material + api/config limit -> active
        - material + count > 0      -> active
        - no material but count > 0 -> active
        - otherwise inactive
        """
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        for rvm_id in self.rvm_ids():
            accepted_cans = (
                self.accepted_cans(rvm_id)
                if stats == (self.data or {}).get("stats")
                else self.safe_int((stats.get(rvm_id, {}) or {}).get(KEY_ACCEPTED_CANS))
            )
            accepted_pet = (
                self.accepted_pet(rvm_id)
                if stats == (self.data or {}).get("stats")
                else self.safe_int((stats.get(rvm_id, {}) or {}).get(KEY_ACCEPTED_PET))
            )
            accepted_glass = (
                self.accepted_glass(rvm_id)
                if stats == (self.data or {}).get("stats")
                else self.safe_int((stats.get(rvm_id, {}) or {}).get(KEY_ACCEPTED_GLASS))
            )
